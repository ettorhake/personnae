...
function handleCSVImport() {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            rows.forEach(row => {
                // Handle possible quotes in CSV
                const [field, ...values] = row.split(',').map(item => item.trim().replace(/^["']|["']$/g, ''));
                const value = values.join(',').trim(); // Rejoin in case the value contains commas
                if (field && value) {
                    console.log('Found field:', field, 'with value:', value); // Debug line
...
function updateFormWithData(data) {
    // Mapping des champs du CSV vers les champs du formulaire
    const mapping = {
        'nom': 'nom',
        'âge': 'age',
        'poste': 'poste',
        ...
        'objectifs professionnels': 'objectifs',
        'besoins fonctionnels': 'besoins',
        'frustrations': 'frustrations',
        'opportunités ux': 'opportunites',
        'niveau connaissance': 'connaissance',
        'niveau comprehension': 'comprehension',
        'niveau distraction': 'distraction'
    };

    // Mettre à jour le formulaire
    Object.entries(mapping).forEach(([csvField, formField]) => {
        const input = personaForm.querySelector(`[name="${formField}"]`);
        if (input && data[csvField.toLowerCase()]) {
            input.value = data[csvField.toLowerCase()];
            updatePreview(formField, data[csvField.toLowerCase()]);
        }
    });
}
